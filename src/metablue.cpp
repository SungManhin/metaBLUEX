...
arma::vec S1_wls(arma::vec summary, double n) {
  ...
  double sigma_hat = w3 * (summary(2) - summary(0)) /
    (2 * R::qnorm((n - 0.375) / (n + 0.25), 0, 1, 1, 0));
...
arma::vec S2_wls(arma::vec summary, double n) {
  ...
  double sigma_hat =
    (1 - w3) * (summary(2) - summary(0)) /
      (2 * R::qnorm((0.75 * n - 0.125) / (n + 0.25), 0, 1, 1, 0));
...
arma::vec S3_wls(arma::vec summary, double n) {
  ...
  double mu_hat = w1 * (summary(0) + summary(4)) / 2 +
    w2 * (summary(1) + summary(3)) / 2 +
    (1 - w1 - w2) * summary(2);
  double sigma_hat =
    w3 * (summary(4) - summary(0)) /
      (2 * R::qnorm((n - 0.375) / (n + 0.25), 0, 1, 1, 0)) +
        (1 - w3) * (summary(3) - summary(1)) /
          (2 * R::qnorm((0.75 * n - 0.125) / (n + 0.25), 0, 1, 1, 0));
...
double Q3(double r, int n) {
  double gamma = -R::qnorm(r / (n + 1), 0, 1, 1, 0) / sqrt(2);
  return 2 * sqrt(2) * pow(datum::pi, 1.5) * exp(3 * gamma * gamma) *
    (1 + 4 * gamma * gamma);
}

double Q4(double r, int n) {
  double gamma = -R::qnorm(r / (n + 1), 0, 1, 1, 0) / sqrt(2);
  return -4 * sqrt(2) * pow(datum::pi, 2) * gamma * exp(4 * gamma * gamma) *
    (7 + 12 * gamma * gamma);
}
...
double Expectation_yang(double r, int n) {
  ...
  return R::qnorm(pr, 0, 1, 1, 0) + pr * qr * Q2(r, n) / (2 * n + 4) +
    pr * qr * value / ((n + 2) * (n + 2));
}
...
double Covariance_yang(double r, double s, int n) {
  ...
  double qs = 1 - ps;
  double value1 =
    (qr - pr) * Q2(r, n) * Q1(s, n) + (qs - ps) * Q1(r, n) * Q2(s, n);
  double value2 = pr * qr * Q3(r, n) * Q1(s, n) / 2 +
    ps * qs * Q3(s, n) * Q1(r, n) / 2 +
    pr * qs * Q2(r, n) * Q2(s, n) / 2;
  
  return pr * qs * Q1(r, n) * Q1(s, n) / (n + 2) +
    pr * qs * (value1 + value2) / ((n + 2) * (n + 2));
}
  ...
  for (int i = 0; i < len; i++) {
    for (int j = 0; j < len; j++)
      if (i > j) {
        ...
        cov(i, j) = Covariance_yang(index(i), index(j), n);
      }
  }
  return cov;
}
...
arma::vec S3_yang(arma::vec summary, double n) {
  int len = summary.n_elem;
  arma::vec index = {1, floor(n / 4) + 1, floor(n / 2) + 1,
                     floor(3 * n / 4) + 1, n};
  arma::mat cov = zeros(len, len);
  arma::mat alpha = zeros(2, len);
...
arma::vec quintiles_yang(arma::vec summary, double n) {
  int len = summary.n_elem;
  arma::vec index = {floor(n / 5) + 1, floor(2 * n / 5) + 1,
                     floor(3 * n / 5) + 1, floor(4 * n / 5) + 1};
  arma::mat cov = zeros(len, len);
  arma::mat alpha = zeros(2, len);
...
arma::vec deciles_yang(arma::vec summary, double n) {
  int len = summary.n_elem;
  arma::vec index = {
    floor(n / 10) + 1,     floor(2 * n / 10) + 1, floor(3 * n / 10) + 1,
    floor(4 * n / 10) + 1, floor(5 * n / 10) + 1, floor(6 * n / 10) + 1,
    floor(7 * n / 10) + 1, floor(8 * n / 10) + 1, floor(9 * n / 10) + 1};
  arma::mat cov = zeros(len, len);
  arma::mat alpha = zeros(2, len);
...
double Q3_logit(double r, int n) {
  double pr = r / (n + 1);
  double s = sqrt(3) / datum::pi;
  return 2 * s * (3 * pr * pr - 3 * pr + 1) /
    (pr * pr * pr * (1 - pr) * (1 - pr) * (1 - pr));
}

double Q4_logit(double r, int n) {
  double pr = r / (n + 1);
  double s = sqrt(3) / datum::pi;
  return 6 * s * (4 * pr * pr * pr - 6 * pr * pr + 4 * pr - 1) /
    (pr * pr * pr * pr * (1 - pr) * (1 - pr) * (1 - pr) * (1 - pr));
}
...
double Expectation_yang_logit(double r, int n) {
  ...
  return 0 + s * log(pr / qr) + pr * qr * Q2_logit(r, n) / (2 * n + 4) +
    pr * qr * value / ((n + 2) * (n + 2));
}
...
double Covariance_yang_logit(double r, double s, int n) {
  ...
  double ps = s / (n + 1);
  double qs = 1 - ps;
  double value1 = (qr - pr) * Q2_logit(r, n) * Q1_logit(s, n) +
    (qs - ps) * Q1_logit(r, n) * Q2_logit(s, n);
  double value2 = pr * qr * Q3_logit(r, n) * Q1_logit(s, n) / 2 +
    ps * qs * Q3_logit(s, n) * Q1_logit(r, n) / 2 +
    pr * qs * Q2_logit(r, n) * Q2_logit(s, n) / 2;
  
  return pr * qs * Q1_logit(r, n) * Q1_logit(s, n) / (n + 2) +
    pr * qs * (value1 + value2) / ((n + 2) * (n + 2));
}
  ...
  for (int i = 0; i < len; i++) {
    for (int j = 0; j < len; j++)
      if (i > j) {
        ...
        cov(i, j) = Covariance_yang_logit(index(i), index(j), n);
      }
  }
  return cov;
}
...
double Q2_lap(double r, int n) {
  double pr = r / (n + 1);
  if (pr >= 0.5) {
    return 1 / (sqrt(2) * (1 - pr) * (1 - pr));
  } else {
    return -1 / (sqrt(2) * pr * pr);
  }
}
...
double Q3_lap(double r, int n) {
  double pr = r / (n + 1);
  if (pr >= 0.5) {
    return sqrt(2) / ((1 - pr) * (1 - pr) * (1 - pr));
  } else {
    return sqrt(2) / (pr * pr * pr);
  }
}
...
double Q4_lap(double r, int n) {
  double pr = r / (n + 1);
  if (pr >= 0.5) {
    return 3 * sqrt(2) / ((1 - pr) * (1 - pr) * (1 - pr) * (1 - pr));
  } else {
    return -3 * sqrt(2) / (pr * pr * pr * pr);
  }
}
...
double Expectation_yang_lap(double r, int n) {
  ...
  return 0 - sign(pr - 0.5) * log(1 - 2 * abs(pr - 0.5)) / sqrt(2) +
    pr * qr * Q2_lap(r, n) / (2 * n + 4) +
    pr * qr * value / ((n + 2) * (n + 2));
}
...
double Covariance_yang_lap(double r, double s, int n) {
  ...
  double ps = s / (n + 1);
  double qs = 1 - ps;
  double value1 = (qr - pr) * Q2_lap(r, n) * Q1_lap(s, n) +
    (qs - ps) * Q1_lap(r, n) * Q2_lap(s, n);
  double value2 = pr * qr * Q3_lap(r, n) * Q1_lap(s, n) / 2 +
    ps * qs * Q3_lap(s, n) * Q1_lap(r, n) / 2 +
    pr * qs * Q2_lap(r, n) * Q2_lap(s, n) / 2;
  
  return pr * qs * Q1_lap(r, n) * Q1_lap(s, n) / (n + 2) +
    pr * qs * (value1 + value2) / ((n + 2) * (n + 2));
}
  ...
  for (int i = 0; i < len; i++) {
    for (int j = 0; j < len; j++)
      if (i > j) {
        ...
        cov(i, j) = Covariance_yang_lap(index(i), index(j), n);
      }
  }
  return cov;
}
...
double Expectation_bala(double r, int n) {
  ...
  return R::qnorm(pr, 0, 1, 1, 0) +
    Q2(r, n) * pr * ((r + 1) / (n + 2) - pr) / 2;
}
...
double Covariance_bala(double r, double s, int n) {
  if (((r == 1) && (s == 1)) || ((r == n) && (s == n))) {
    ...
    double term2 =
      Q2(r, n) * R::qnorm(qs, 0, 1, 1, 0) * pr * ((r + 1) / (n + 2) - pr) / 2;
    double term3 = Q1(r, n) * Q1(s, n) * pr * ((s + 1) / (n + 2) - qs);
    double term4 =
      R::qnorm(pr, 0, 1, 1, 0) * Q2(s, n) * qs * ((s + 1) / (n + 2) - qs) / 2;
    
    return term1 + term2 + term3 + term4 -
      Expectation_bala(r, n) * Expectation_bala(s, n);
  }
}
  ...
  for (int i = 0; i < len; i++) {
    for (int j = 0; j < len; j++)
      if (i > j) {
        ...
        cov(i, j) = Covariance_bala(index(i), index(j), n);
      }
  }
  return cov;
}
...
arma::vec S1_lap(arma::vec summary, double n) {
  ...
  double mu_hat = w1 * (summary(0) + summary(2)) / 2 + (1 - w1) * summary(1);
  double sigma_hat = w3 * (summary(2) - summary(0)) /
    (2 / (-0.0084688 + 1.46221 * pow(log(n), -0.988499)));
...
arma::vec S2_lap(arma::vec summary, double n) {
  ...
  double mu_hat = w2 * (summary(0) + summary(2)) / 2 + (1 - w2) * summary(1);
  double sigma_hat = (1 - w3) * (summary(2) - summary(0)) /
    (2 / (2.0424 + 2.91911 * pow(n, -1.20853)));
...
arma::vec S3_lap(arma::vec summary, double n) {
  ...
  double w3 =
    9.30594 / (10.8703 + 0.064734 * n + 0.581161 * pow(log(n), 1.95934));
  arma::vec beta_hat(2, fill::zeros);
  
  double mu_hat = w1 * (summary(0) + summary(4)) / 2 +
    w2 * (summary(1) + summary(3)) / 2 +
    (1 - w1 - w2) * summary(2);
  double sigma_hat = w3 * (summary(4) - summary(0)) /
    (2 / (-0.0084688 + 1.46221 * pow(log(n), -0.988499))) +
      (1 - w3) * (summary(3) - summary(1)) /
        (2 / (2.0424 + 2.91911 * pow(n, -1.20853)));
...
arma::vec S1_logit(arma::vec summary, double n) {
  ...
  double mu_hat = w1 * (summary(0) + summary(2)) / 2 + (1 - w1) * summary(1);
  double sigma_hat = w3 * (summary(2) - summary(0)) /
    (2 / (0.182894 + 1.19771 * pow(n, -0.426478)));
...
arma::vec S2_logit(arma::vec summary, double n) {
  ...
  double mu_hat = w2 * (summary(0) + summary(2)) / 2 + (1 - w2) * summary(1);
  double sigma_hat = (1 - w3) * (summary(2) - summary(0)) /
    (2 / (1.65277 + 3.1854 * pow(n, -1.13223)));
...
arma::vec S3_logit(arma::vec summary, double n) {
  ...
  double w3 =
    448.738 / ((404.359 + 16.8838 * pow(n, 0.660158)) * pow(n, 0.143355));
  arma::vec beta_hat(2, fill::zeros);
